<x-app-layout>
    ...
    <div class="mb-6 flex justify-between items-center">
        <div class="flex-1 max-w-lg">
            <form action="{{ route('doctors.index') }}" method="GET" class="relative flex items-center gap-2">
                <div class="relative flex-1">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by Name, Specialization, or Phone..." class="w-full pl-10 pr-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:border-indigo-500">
                ...
                @if(request('search'))
                    <a href="{{ route('doctors.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors">Reset</a>
                @endif
            </form>
        </div>
        <a href="{{ route('doctors.create') }}" class="ml-4 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors flex items-center">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach ($doctors as $doctor)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">Rp {{ number_format($doctor->consultation_fee, 0, ',', '.') }}</td>
